			...
			if string(record[0]) != "jid" {
				...
				if controllerKind == "deployment" {
					...
					//go CreateAndManageDemand(controllerName, nodePort, frequency, replicaOrParallelismInt, experimentDuration-timeRef, &wg)
                                        go CreateAndManageDemand(controllerName, nodePort, serviceAttackRate, replicaOrParallelismInt, experimentDuration-timeRef, &wg)
					...
					fmt.Println("Reading new task...")
					fmt.Printf("Job %s, cpu: %v, mem: %v, slo: %s, paralelism: %d, completions: %d, qosMeasuring: %s\n", controllerName, cpuReq, memReq, slo, replicaOrParallelism, completions, qosMeasuring)
...
func CreateAndManageDemand(controllerName string, nodePort int, frequency int, numberOfReplicas int, expectedRuntime int, wg *sync.WaitGroup) {
	...
	duration :=  time.Duration(expectedRuntime - 3) * time.Second

	rate := vegeta.Rate{Freq: freq, Per: time.Second}
	targeter := vegeta.NewStaticTargeter(vegeta.Target{
		Method: "GET",
		URL:    "http://localhost:" + strconv.Itoa(nodePort) + "/",
                //URL:    "http://localhost:" + strconv.Itoa(nodePort) + "/factorial.php" + "?number=50",
	})
	...
	pod := v1.PodTemplateSpec{
		...
		Spec: v1.PodSpec{
			...
			Containers: []v1.Container{{Name: controllerRefName,
				Image: "viniciusbds/factorial:v0.0.2",  //factorial with default connections = 2014
                                //Image: "nginx:1.15",
				//Image: "golang:1.11",
